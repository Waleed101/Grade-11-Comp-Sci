...
public class Example
{
  public static void main (String[] args) {
    ...
    for (int i=0; i < Math.random()*100; i++) {
      joe.rt((Math.random())*1000).fd(200);                 // turn 90 degrees to the right, then 
                                          // move forward 200 pixels.
    }
    joe.setPenColor(Color.yellow);           // set the pen color to red.
    joe.pu();                             // lifts the pen off the canvas
    joe.bk(50).rt(90).bk(50).lt(90).pd();
    for (int i=0; i < 4; i++) {
      joe.rt(90).fd(100);
    }
